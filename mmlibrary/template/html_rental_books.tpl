...
<h3>{{ user.disp_name }}(ID:{{ user.id }})</h3>
{% if user.rental_books.filter_setting.is_type_none -%}
{{ user.disp_name }}の借りてる本です。
{%- elif user.rental_books.filter_setting.is_type_expired -%}
{{ user.disp_name }}の借りた次の本は、期限が切れています！
{%- elif user.rental_books.filter_setting.is_type_expire -%}
{{ user.disp_name }}の借りた次の本は、返却期限まであと{{ user.rental_books.filter_setting.xdays }}日を切りました。
{% endif -%}
{% else -%}
{{ user.disp_name }}(ID:{{ user.id }})
{%- if user.rental_books.filter_setting.is_type_none -%}
の借りてる本はありません。
{%- elif user.rental_books.filter_setting.is_type_expired -%}
の借りてる本で期限切れの本はありません。
